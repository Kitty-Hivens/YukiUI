...
See `dots/.config/quickshell/yuki/translations/tools`
...
- Quickshell-specific LSP setup: Run `touch ~/.config/quickshell/yuki/.qmlls.ini` for proper LSP support.
...
- For the shell:
  - Open `~/.config/quickshell/yuki` in your code editor.
  - In a terminal run `pkill qs; qs -c yuki` to start the shell in the terminal (for logs).
