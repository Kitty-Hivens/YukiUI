...
Example: In `~/‎.config/quickshell/yuki/screenshot.qml`:
```qml
Process {
...
In this example, in `~/‎.config/quickshell/yuki/screenshot.qml` we should modify `find_regions.py` to the wrapper script `find-regions-venv.sh`:
```qml
Process {
id: imageDetectionProcess
