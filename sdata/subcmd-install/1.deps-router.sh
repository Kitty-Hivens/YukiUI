...
if [[ "$INSTALL_VIA_NIX" == "true" ]]; then
  ...
  if ! [[ "${TARGET_ID}" = "arch" ]]; then
    ...
    if [[ "${tmp_update_status}" =~ ^(OUTDATED|EMPTY_TARGET|EMPTY_SOURCE|FORCE_OUTDATED|WIP)$ ]]; then
      ...
      printf "\n"
      printf "${STY_RED}It's highly recommended to compare the two lists of commit history before continue.${STY_RST}\n"
      printf "   ${STY_UNDERLINE}https://github.com/Kitty-Hivens/YukiUI/commits/main/sdata/dist-arch${STY_RST}\n"
      printf "   ${STY_UNDERLINE}https://github.com/Kitty-Hivens/YukiUI/commits/main/sdata/dist-${TARGET_ID}${STY_RST}\n"
      printf "\n"
      ...
      printf "${STY_PURPLE}${STY_BOLD}Please do not open an issue about it: the supported distros are Arch-based and NixOS.${STY_RST}\n"
      printf "\n"
